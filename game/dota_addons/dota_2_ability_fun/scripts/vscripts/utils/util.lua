...
function Util:FindAllOwnedUnits(player)
	...
	local pid = type(player) == "number" and player or player:GetPlayerID()
	local hero = PlayerResource:GetSelectedHeroEntity(pid)
	local units = FindUnitsInRadius(PlayerResource:GetTeam(pid), Vector(0, 0, 0), nil, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, FIND_ANY_ORDER, false)
	for _,v in ipairs(units) do
		if type(player) == "number" and ((v.GetPlayerID ~= nil and v:GetPlayerID() or v:GetPlayerOwnerID()) == pid) or v:GetPlayerOwner() == player then
			if not v:HasModifier("modifier_dummy_unit") and v ~= hero then
				table.insert(summons, v)
...
function Util:CastAdditionalAbility(caster, ability, target, delay, channelData)
	...
	local previous_target = unit:GetCursorCastTarget()
	local previous_cursor_position = unit:GetCursorPosition()
	...
	unit:SetCursorCastTarget(previous_target)
	unit:SetCursorPosition(previous_cursor_position)
