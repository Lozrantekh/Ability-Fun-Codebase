...
LinkLuaModifier("modifier_multicast_lua", "abilities/heroes/ogre_magi/multicast.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_multicast_grow", "abilities/heroes/ogre_magi/modifier_multicast_grow.lua", LUA_MODIFIER_MOTION_NONE)
...
if IsServer() then
	...
	local MULTICAST_ABILITIES = {
		...
		ogre_magi_smash = MULTICAST_TYPE_NONE,
		...
		item_arcane_blink_lua = MULTICAST_TYPE_NONE,
		...
		item_mask_of_sange_and_yasha = MULTICAST_TYPE_NONE,
		...
		item_blade_mail_lua = MULTICAST_TYPE_NONE,
		...
		item_revenants_brooch_lua = MULTICAST_TYPE_NONE,
		...
		ancient_apparition_ice_blast_release_lua = MULTICAST_TYPE_NONE,
		...
		alchemist_unstable_concoction_lua = MULTICAST_TYPE_NONE,
		...
		phoenix_supernova_lua = MULTICAST_TYPE_NONE,
		tusk_snowball = MULTICAST_TYPE_NONE,
		ancient_apparition_ice_blast_lua = MULTICAST_TYPE_NONE,
		...
		faceless_void_time_walk_lua = MULTICAST_TYPE_NONE,
		...
		primal_beast_pulverize = MULTICAST_TYPE_NONE,
		muerta_pierce_the_veil = MULTICAST_TYPE_NONE,
		windrunner_focusfire = MULTICAST_TYPE_NONE,
		terrorblade_sunder_lua = MULTICAST_TYPE_NONE,
		weaver_time_lapse = MULTICAST_TYPE_NONE,
		...
		centaur_mount = MULTICAST_TYPE_NONE,
		faceless_void_time_walk_reverse_lua = MULTICAST_TYPE_NONE,
		muerta_the_calling = MULTICAST_TYPE_NONE,
		...
		night_stalker_darkness = MULTICAST_TYPE_NONE,
		...
		legion_commander_duel_lua = MULTICAST_TYPE_SAME,
		bounty_hunter_shuriken_toss_lua = MULTICAST_TYPE_SAME,
		tusk_walrus_kick = MULTICAST_TYPE_DIFFERENT,
		skywrath_mage_mystic_flare = MULTICAST_TYPE_DIFFERENT,
		doom_bringer_devour = MULTICAST_TYPE_DIFFERENT,
		item_hand_of_midas = MULTICAST_TYPE_DIFFERENT,
		ogre_magi_fire_shield_lua = MULTICAST_TYPE_DIFFERENT,
		...
		witch_doctor_voodoo_switcheroo = MULTICAST_TYPE_SUMMON,
		...
		hoodwink_decoy_lua = MULTICAST_TYPE_NONE,
		bane_fiends_grip_lua = MULTICAST_TYPE_NONE,
		keeper_of_the_light_chakra_magic_lua = MULTICAST_TYPE_NONE,
		warlock_upheaval_lua = MULTICAST_TYPE_NONE,
	}

	-- For abilities with DOTA_UNIT_TARGET_TEAM_CUSTOM
	local override_unit_target_team = {
		doom_bringer_devour = DOTA_UNIT_TARGET_TEAM_ENEMY,
	}

	local override_unit_target_type = {
	...
	function GetMulticastFlags(caster, ability, multicast_type)
		...
		if multicast_type ~= MULTICAST_TYPE_SAME then
			...
			if override_unit_target_team[ability_name] then abilityTarget = override_unit_target_team[ability_name] end
			...
			elseif abilityTargetType == DOTA_UNIT_TARGET_CREEP and ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT) then 
				abilityTargetType = DOTA_UNIT_TARGET_CREEP + DOTA_UNIT_TARGET_HERO 
			end

			if override_unit_target_type[ability_name] then abilityTargetType = override_unit_target_type[ability_name] end
	...
	function CastMulticastedSpellInstantly(caster, ability, target, multicast_flag_data, multicast_casted_data, delay, channelData)
		...
		for k, v in pairs(candidates) do
			if caster:CanEntityBeSeenByMyTeam(v) and (not target_exceptions[v:GetUnitName()]) and (ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT) or (not ability:HasBehavior(DOTA_ABILITY_BEHAVIOR_POINT) and not v:IsUntargetableFrom(caster))) then
