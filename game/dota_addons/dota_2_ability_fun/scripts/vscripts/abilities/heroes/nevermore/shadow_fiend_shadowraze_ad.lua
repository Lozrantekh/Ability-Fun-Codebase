shadow_fiend_shadowraze_ad = class({})

LinkLuaModifier("modifier_shadow_fiend_shadowraze_ad", "modifiers/heroes/nevermore/modifier_shadow_fiend_shadowraze_ad", LUA_MODIFIER_MOTION_NONE)

function shadow_fiend_shadowraze_ad:GetCastAnimation()
    if IsServer() then
...
function shadow_fiend_shadowraze_ad:OnSpellStart()
    if IsServer() then
        ...
        for _,enemy in pairs(enemies) do
            if enemy and not enemy:IsInvulnerable() then
                ...
                local modifier = enemy:FindModifierByNameAndCaster("modifier_shadow_fiend_shadowraze_ad", self:GetCaster())
                if modifier then
                ...
                if not modifier then
                    enemy:AddNewModifier(
                        self:GetCaster(),
                        self,
                        "modifier_shadow_fiend_shadowraze_ad",
...
function shadow_fiend_shadowraze_ad:PlayEffects( position, radius )
	-- get resources
